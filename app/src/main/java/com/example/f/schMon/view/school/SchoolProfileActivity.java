...
import com.example.f.schMon.view.report.RepActivity;
...
//___________________End option menu______________________________________
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Helper.setFont(context, new View[]{
    ...
    private void bindViewData() {
        schNmeTxt.setText("School Name: " + Helper.toCamelCase(s.getSchName()));
        ...
        poNmeTxt.setText("PO: " + Helper.toCamelCase(poNme));

        if (tech != null) {
            if (!tech.tName.equals(""))
                teacherTxt.setText("Teacher: " + Helper.underscoreToSpacedFormat(tech.tName));
        ...
        if (lpm != null) {
            ...
            try {
                p1 = Math.round(Float.parseFloat(Helper.floatToZeroDecimalPlace(lpm.getLastEvalPerform())));
                p2 = Math.round(Float.parseFloat(Helper.floatToZeroDecimalPlace(lpm.getLastRepPerform())));
                ps1 = Helper.floatToZeroDecimalPlace(lpm.getLastEvalPerform());
                ps2 = Helper.floatToZeroDecimalPlace(lpm.getLastRepPerform());
    ...
    public void onClickperfReportCircle(View view) {
        List<RepMdlin> listTemp = DAO2.getAllReportofASchool(schID);
        Log.d(TAG,listTemp.toString());

        if (listTemp.size() > 0) {
            date =listTemp.get(0).getDateReport();
            Intent intent = new Intent(context, RepActivity.class);
            intent.putExtra("schID", schID);
            intent.putExtra("gradeID", gradeID);
            ...
            if (Helper.isReportOfThisSchoolSynced(listTemp))
        ...
        //String date = DAO2.lastDateForASchool(schID, DB.report_admin.table, DB.report_admin.sch_id, DB.report_admin.date_report);
    ...
    public void onClickperfEvaluationCircle(View view) {
        ...
        if (date != null) {
            ...
            if (Helper.isEvaluationOfThisSchoolSynced(listTemp))
    ...
    public void onStartEvalClick(String schID, String gradeID, String date, String attendance) {
        this.date = date;
        dateInputted = Helper.isDateInputted(date);
