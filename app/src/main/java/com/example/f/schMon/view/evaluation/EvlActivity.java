...
public class EvlActivity extends AppCompatActivity implements DatePickerFragment.DateDialogListner {
    ...
    private boolean isSynced, resultFromActivity;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.info_menu:
                ...
                if (list.size() > 0) {
                    if (list.get(0) != null) {
                        isSynced = Helper.isEvaluationOfThisSchoolSynced(list);
                        bundle.putParcelable("data", Helper.makeInfoMdlForInfoDialogFromEvlModelOrRepMdl(list.get(0), isSynced, schID));
                    }
                }
...
//___________________End option menu______________________________________
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Helper.setFont(context, new View[]{evlSubmitBtn});
    ...
    private void changeItemDataInList(int index, EvlModelin changedData) {
        Helper.formatEvlDataToShowAns(changedData);
    ...
    private void submitAll() {
        Helper.getAlertDialog(context, Helper.getAnsweredQuesFromEvlList(list).toString());
    }
    ...
    public void onDateClick(DialogFragment dialog, boolean dateInputted, Calendar calendar) {
       ...
       /* list = DAO2.getActiveEvaluatioQues("");
        ...
        adapterRecycler = new EvlAdapterRecycler(context, schID, gradeID, date, attendance, openMode, list,isSynced);
        recyclerView.setAdapter(adapterRecycler);
    }


    private void setSyncStatusFalseWhenLoadinNewMode(List<EvlModelin> list) {
        for (EvlModelin e : list) {
            if (e.getPerf() == null)
                e.setSynced("0");
            else {
                if (e.getPerf().equals(""))
    ...
    class LoadData extends AsyncTask<String, Void, List<EvlModelin>> {
        ...
        protected List<EvlModelin> doInBackground(String... strings) {
            ...
            try {
                ...
                if (openMode.equals(Global.NewMode)) {
                    ...
                    Helper.setQuesNoForAnsActivity(list);
                    setSyncStatusFalseWhenLoadinNewMode(list);
                ...
                //_____________ edit mode _______________________
                    ...
                    unAnsQuesList = Helper.quesNotAnsweredPreviousEvaluation(allQues, ansQuesList);

                    list.clear();
                    list.addAll(ansQuesList);
                    list.addAll(unAnsQuesList);
                    Helper.setQuesNoForAnsActivity(list);
                    list = Helper.formatEvlDataToShowAnsInList(list);
        ...
        protected void onPostExecute(List<EvlModelin> evlModelins) {
            recyclerView.setLayoutManager(new LinearLayoutManager(context));
            adapterRecycler = new EvlAdapterRecycler(context, schID, gradeID, date, attendance, openMode, listFiltered,isSynced);
            recyclerView.setAdapter(adapterRecycler);
    ...
    private void loadData() {
        new AsyncTask<String, Void, List<EvlModelin>>() {
            ...
            protected void onPostExecute(List<EvlModelin> evlModelins) {
                recyclerView.setLayoutManager(new LinearLayoutManager(context));
                adapterRecycler = new EvlAdapterRecycler(context, schID, gradeID, date, attendance, openMode,
                        list,isSynced);
                recyclerView.setAdapter(adapterRecycler);
                ...
                //Helper.getAlertDialog(context, allQues.toString());
                //Helper.getAlertDialog(context, ansQuesList.toString());
                //Helper.getAlertDialog(context, Helper.quesNotAnsweredPreviousEvaluation(allQues, ansQuesList).toString());
