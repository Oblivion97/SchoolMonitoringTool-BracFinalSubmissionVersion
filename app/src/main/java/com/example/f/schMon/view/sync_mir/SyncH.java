...
public class SyncH {
    ...
    public static List<RepA> getReportAnsForSync(boolean onlyUnsynced/**if true only unsynced reps if false all reps*/) {
        ...
        if (c.moveToFirst()) {
            do {
                r = new RepA();
                r.mId = c.getString(c.getColumnIndex(DB.report_admin._ID));
                r.mInstituteId = c.getString(c.getColumnIndex(DB.report_admin.sch_id));
                r.mInfrastructureQuestionId = c.getString(c.getColumnIndex(DB.report_admin.server_id_ques));
                r.mSubmittedDate = c.getString(c.getColumnIndex(DB.report_admin.date_report));
                r.mAnswer = Integer.parseInt(c.getString(c.getColumnIndex(DB.report_admin.answer)));
                if (c.getString(c.getColumnIndex(DB.report_admin.priority)) != null) {
                    if (!c.getString(c.getColumnIndex(DB.report_admin.priority)).equals(""))
                        r.mPriority = Integer.parseInt(c.getString(c.getColumnIndex(DB.report_admin.priority)));
                }
                r.mDetails = c.getString(c.getColumnIndex(DB.report_admin.details));
                r.mLocalId = c.getString(c.getColumnIndex(DB.report_admin.local_id_answer));
                r.mModifiedDate = c.getString(c.getColumnIndex(DB.report_admin.time_lm));
                r.mTargetDate = c.getString(c.getColumnIndex(DB.report_admin.date_target));
                r.mSolveDate = c.getString(c.getColumnIndex(DB.report_admin.date_solve));


                r.mQuestionName = c.getString(c.getColumnIndex(DB.report_admin.question));
                r.ques_category_id = c.getString(c.getColumnIndex(DB.report_admin.ques_category_id));
                r.category = c.getString(c.getColumnIndex(DB.report_admin.category));
                r.mYesComment = c.getString(c.getColumnIndex(DB.report_admin.yesComment));
                r.mIsProblem = c.getString(c.getColumnIndex(DB.report_admin.isProblem));
    ...
    public static int updateDB_on_SyncSuccessRep(List<RepA> list) {
        ...
        for (RepA r : list) {
            ...
            selA = new String[]{r.mInstituteId, r.mSubmittedDate, r.mInfrastructureQuestionId};
    ...
    public static List<String> setOtherDataSyncLastRep(List<RepA> repAS) {
        boolean f = false;
        List<String> listOfNotFoundQues = new ArrayList<>();

        for (int i = 0; i < repAS.size(); i++) {
            RepA repA = repAS.get(i);
            if (repA.getmQuestionName() == null) {
                RepQ repQ = DAO2.getRepQuesyQuesID(repA.getmInfrastructureQuestionId());
                repA.setmQuestionName(repQ.getTitle());
                repA.setmCategoryId(repQ.getInfrastructureQuestionCategoryId());
                repA.setmCategoryName(repQ.getInfrastructureQuestionCategoryName());
                if (repQ == null)
                    listOfNotFoundQues.add(repA.getmInfrastructureQuestionId());
            }

            School s = DAO.getInfoOfASchool(repA.mInstituteId);
            ...
            //setting notification for later solving the problem
            if (repA.getmAnswer() == 0)
                repA.setNotify("1");
            else repA.setNotify("0");
    ...
    public static void updatePerformTableAfterSyncLastReport() {  //environment
    ...
    public static void updatePerformTableAfterSyncLastEvalution() {//evaluation
        ...
        for (int i = 0; i < listList.size(); i++) {
            ...
            School s = DAO.getInfoOfASchool(list.get(0).getSchID());
            perf = Helper.calcEvlPerfOfAEvalReport(list);
    ...
    public static List<String> setOtherDataSyncLastEvl(List<EvlA> evlAs) {
        boolean f = false;
        List<String> listOfNotFoundQues = new ArrayList<>();
    ...
    public static boolean isServerTimeSystemTimeSame() throws IOException {
        ...
        TimeMdl timeMdlCall = api.getTimeFromServer(App.getU(), App.getP()).execute().body();
    ...
    public static List<String> unSyncSchools() {
        ...
        api.getSchList(App.getU(), App.getP(), "0").enqueue(new Callback<SchMdlList>() {
            @Override
            public void onResponse(Call<SchMdlList> call, Response<SchMdlList> response) {
                api.getSchList(App.getU(), App.getP(), response.body().getTotal()).enqueue(new Callback<SchMdlList>() {
                    @Override
                    public void onResponse(Call<SchMdlList> call, Response<SchMdlList> response) {
