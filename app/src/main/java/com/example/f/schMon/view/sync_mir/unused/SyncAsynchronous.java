...
public class SyncAsynchronous {
    ...
    public void schoolSyncAsync() {
        api.getSchList(App.getU(), App.getP(), "0").enqueue(new Callback<SchMdlList>() {
            ...
            public void onResponse(Call<SchMdlList> call, Response<SchMdlList> response) {
                ...
                App.calcDwnload(response.body().toString().length());
                if (response.body().getSuccess().equalsIgnoreCase("1")) {
                    maxSch = response.body().getTotal();
                    api.getSchList(App.getU(), App.getP(), maxSch).enqueue(new Callback<SchMdlList>() {
                        @Override
                        public void onResponse(Call<SchMdlList> call, Response<SchMdlList> response) {
                            dataCost(response.body().toString().length());
                            App.calcDwnload(response.body().toString().length());
    ...
    public void schoolDetails(List<SchMdl> schMdls) {
        for (int i = 0; i < schMdls.size(); i++) {
            api.getSchByID(schMdls.get(i).getId(), App.getU(), App.getP()).enqueue(new Callback<SchMdl_Id_Res>() {
                @Override
    ...
    public void teacherSyncAsync() {
        api.getTehrList(App.getU(), App.getP(), "0").enqueue(new Callback<TehMdlList>() {
            @Override
            public void onResponse(Call<TehMdlList> call, Response<TehMdlList> response) {
                ...
                api.getTehrList(App.getU(), App.getP(), maxTchr).enqueue(new Callback<TehMdlList>() {
                    @Override
                    public void onResponse(Call<TehMdlList> call, Response<TehMdlList> response) {
                        dataCost(response.body().toString().length());
                        App.calcDwnload(response.body().toString().length());
                        // Helper.getAlertDialog(context, String.valueOf(response.body().toString()));
                        if (response.body().getSuccess().equalsIgnoreCase("1")) {
    ...
    public void teacherDetails(List<TchrMdl> tchrMdls) {
        for (int i = 0; i < tchrMdls.size(); i++) {
            api.getTehrByID(tchrMdls.get(i).getId(), App.getU(), App.getP()).enqueue(new Callback<TchrMdl_id_Res>() {
                @Override
    ...
    public void studentSyncAsync() {
        api.getStdList(App.getU(), App.getP(), "0").enqueue(new Callback<StdMdlList>() {
            @Override
            public void onResponse(Call<StdMdlList> call, Response<StdMdlList> response) {
                dataCost(response.body().toString().length());
                App.calcDwnload(response.body().toString().length());
                maxStd = response.body().getTotal();
                api.getStdList(App.getU(), App.getP(), maxStd).enqueue(new Callback<StdMdlList>() {
                    @Override
                    public void onResponse(Call<StdMdlList> call, Response<StdMdlList> response) {
                        dataCost(response.body().toString().length());
                        App.calcDwnload(response.body().toString().length());
    ...
    public void studentDetails(List<StdMdl> stdMdls) {
        for (int i = 0; i < stdMdls.size(); i++) {
            api.getStdByID(stdMdls.get(i).getId(), App.getU(), App.getP()).enqueue(new Callback<StdMdl_id_Res>() {
                @Override
