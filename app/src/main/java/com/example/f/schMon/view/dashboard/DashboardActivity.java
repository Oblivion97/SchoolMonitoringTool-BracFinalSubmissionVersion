...
import com.example.f.schMon.view.previous_log.LogEditActivityTab;
import com.example.f.schMon.view.notificationProblem.NotifActivity;
import com.example.f.schMon.view.notificationProblem.NotifService;
...
public class DashboardActivity extends AppCompatActivity {
    ...
    @BindView(R.id.dash_designation)
            ...
            switch (item.getItemId()) {
                ...
                case R.id.navigation_notific:
                    if (Global.isPO_userRole())
                        ...
                        startActivity(new Intent(context, NotifSlvActivity.class));
                    return true;
            }
            return false;
        }
...
//___________________start option menu______________________________________
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Helper.setFont(context, new View[]{userNameTV, userMobileTV, userdesignationTV, progTitle, dash_tv_attendance,
                dash_tv_activities, dash_tv_infrastructure, dash_listValue, dash_listName, noDataTV, noDataBtn, visitedSchTV
                , visitedSchTitle});
    ...
    void setPOInfos() {
        ...
        if (userM.getFirstName() == null && userM.getMiddleName() == null && userM.getLastName() == null)
            userNameTV.setText(Helper.toCamelCase(userM.getUsername()).replaceAll("_", " "));
        else if (userM.getFirstName() != null && userM.getLastName() != null)
            userNameTV.setText(Helper.toCamelCase(userM.getFirstName() + " " + userM.getLastName()));
        else
            userNameTV.setText(Helper.toCamelCase(userM.getFirstName() + " " + userM.getMiddleName() + " " + userM.getLastName()));

        userMobileTV.setText(userM.getMobileNumber());

        if (userM.getRoleName() == null)
            userdesignationTV.setText("");
        else
            userdesignationTV.setText(userM.getRoleName());
    }
    ...
    class LoadData extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            visitedSchTV.setText(visitedSch[0] + "/" + visitedSch[1]);
            listView.setAdapter(adapter);

            //default performance is set to report
            progrssPercent = avgPerformanceList.get(2);
            progTitle.setText("Overall Environment");
            //btn color
            dash_evl.setImageResource(R.drawable.ic_evl_dash);
