...
public class DAO2 {
    ...
    public static String lastDateForASchool(String schID, String table, String schIDColumn, String dateColumn) {
        ...
        List<String> lastDates = Helper.lastDates(allDatesForASchool, 1);
        if (lastDates.size() > 0)
    ...
    public static List<RepMdlin> getAllReportofASchool(String schID) {
        ...
        String sel = DB.report_admin.sch_id + " = ?";
        String[] selArg = new String[]{schID};
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                do {
                    ...
                    r.setAnswer(c.getString(c.getColumnIndex(DB.report_admin.answer)));
                    ...
                    r.setCategoryID(c.getString(c.getColumnIndex(DB.report_admin.ques_category_id)));
                    r.setCategoryName(c.getString(c.getColumnIndex(DB.report_admin.category)));
                    r.setYesComment(c.getString(c.getColumnIndex(DB.report_admin.yesComment)));
                    r.setIsProblem(c.getString(c.getColumnIndex(DB.report_admin.isProblem)));
    ...
    public static List<RepMdlin> getAllReportofAllSchool() {
        List<RepMdlin> list = new ArrayList<>();
    ...
    public static List<PerfMdl> getLast_n_RepPerform1School(String schID, int noOfLastEvl) {
        ...
        List<String> dates = Helper.getUniqueDateFromATableForASch(schID, DB.report_admin.table, DB.report_admin.date_report, DB.report_admin.sch_id);
        dates = Helper.lastDates(dates, noOfLastEvl);
        String date, temp;

        for (int i = 0; i < dates.size(); i++) {
            date = dates.get(i);
            List<RepMdlin> repMdlinList = DAO2.getReport1School1Date(schID, date);
            temp = Helper.calcRepPerformForASch(repMdlinList);
    ...
    public static boolean insertRepQues_Got_From_Server(List<RepQ> list) {
        ...
        for (RepQ r : list) {
            //extra data set
            if (r.getLocalId() == null)
                r.setLocalId(Helper.generateUniqueID("rep_ques"));
            ...
            cv.put(DB.admin_ques.time_lm, Helper.currentDate_in_dateFormat1());
    ...
    public static String calcEvlPerformanceOfASch(List<EvlModelin> evlModelins) {
        ...
        float sum = 0, avg = 0;
        for (int i = 0; i < n; i++) {
            sum += Float.parseFloat(Helper.evaluationPerfromCalc(evlModelins.get(i).getAsked(), evlModelins.get(i).getCorrect()));
        }
    ...
    public static List<List<EvlModelin>> getLast_n_Evl1Sch(String schID, int noOfLastEvl) {
        ...
        List<String> dates = Helper.getUniqueDateFromATable(DB.report_acad.table, DB.report_acad.date_report);
        dates = Helper.lastDates(dates, noOfLastEvl);
        for (int i = 0; i < dates.size(); i++) {
    ...
    public static boolean insertNewQuestion(EvalQuesModel ques) {
        ...
        cv.put(DB.acad_ques.grade, ques.getGrade());
        ...
        cv.put(DB.acad_ques.subject, ques.getSubject());
        cv.put(DB.acad_ques.chapter, ques.getChapter());
    ...
    public static boolean insertEvlQues_Got_From_Server(List<EvlQ> list) {
        ...
        for (EvlQ e : list) {
            ...
            if (e.getLocalId() == null)
                e.setLocalId(Helper.generateUniqueID("evl_ques"));

            cv.put(DB.acad_ques._ID, e.getId());
            ...
            cv.put(DB.acad_ques.time_lm, Helper.currentDate_in_dateFormat1());
    ...
    public static List<RepMdlin> getAllExistingProblemsFromReport() {
        SQLiteDatabase db = Global.getDB_with_nativeWay_writable(Global.dbName);
        RepMdlin r;
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                do {
                    ...
                    r.setNotify_his(c.getString(c.getColumnIndex(DB.report_admin.notify_his)));
                    r.setN_update_date(c.getString(c.getColumnIndex(DB.report_admin.n_update_date)));
                    r.setGradeID(c.getString(c.getColumnIndex(DB.report_admin.grade_id)));
                    ...
                    r.setServerIDAns(c.getString(c.getColumnIndex(DB.report_admin.server_id_answer)));
                    // r.setTimeLm(c.getString(c.getColumnIndex(DB.report_admin.time_lm)));
                    //(c.getString(c.getColumnIndex(DB.report_admin.server_id_answer)));
...
//=============================== payments ==========================================================================================
    ...
    public static boolean insertPaymentData(PayMdl payMdl, String schID, String gradeID, String year) {
        ...
        List<PayMdlInner> payMdlInnerList = Helper.PayMdl_to_PayMdlInner(payMdl, schID, gradeID, year);
...
//============================ attendance ===================================================================
    ...
    public static boolean insertAttdnceDashPerform(AttndDashMdl attndDashMdl) {
        boolean flag = true;
        long update;
        SQLiteDatabase db = Global.getDB_with_nativeWay_writable(Global.dbName);
        ContentValues cv = null;

        String performance = Helper.floatToZeroDecimalPlace(attndDashMdl.getAttendancePercentage());

        try {
            cv = new ContentValues();
            cv.put(DB.perform.sch_id, attndDashMdl.getSchoolId());
            cv.put(DB.perform.sch_name, attndDashMdl.getSchoolName());
            cv.put(DB.perform.s_code, attndDashMdl.getSchoolCode());
            cv.put(DB.perform.grade, attndDashMdl.getSchoolGrade());
            cv.put(DB.perform.lastAttenPerform, performance);
            cv.put(DB.perform.lastAttenPerformDate, attndDashMdl.getDate());
            cv.put(DB.perform.edu_type,
                    DAO.getInfoOfASchool(attndDashMdl.getSchoolId()).getEdu_type());


            String where = DB.perform.sch_id + " = ? ";
            String[] whereArg = new String[]{attndDashMdl.getSchoolId()};
            update = db.update(DB.perform.table, cv, where, whereArg);
